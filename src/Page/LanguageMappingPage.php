...
class LanguageMappingPage extends AbstractPage
{
    ...
    public function renderPage()
    {
        ...
        <div class="memsource-admin-section-description"><?php _e('<p>This page gives you the option to map default WordPress language codes to Phrase TMS language codes. See our documentation for the list of all 
                                                                   ...
                                                                   <p>The <b>Restore default</b> option will replace any customized language codes with WordPress default language codes.</p>', 'memsource'); ?></div>
        <?php
        if (isset($_GET['valid']) && $_GET['valid'] === 'false') {
            ?>
            <div class="notice notice-error is-dismissible">
                <p><?php _e('The form is not valid. Each field must be filled and contain a unique code. Please, fill in the form and save again.', 'memsource'); ?></p>
            </div>
            <?php
        }
        ?>
    ...
    public function formSubmit()
    {
        ...
        foreach ($_POST as $wpCode => $memsourceCode) {
            ...
            if (empty($memsourceCode) || in_array($memsourceCode, $usedMemsourceCodes, true)) {
                $wpdb->query('ROLLBACK');
                wp_redirect($_POST['referer'] . '&valid=false'); // phpcs:ignore
                exit;
            }
        ...
        $wpdb->query('COMMIT');
        wp_redirect($_POST['referer'] . '&valid=true'); // phpcs:ignore
        exit;
    }
}
