...
class CustomFieldsPage extends AbstractPage
{
    ...
    public function renderPage()
    {
        ...
        <div class="memsource-admin-section-description">
            <p><?php _e('Select which custom fields should be exported for translation with a post or a page. Whenever a new theme or a page builder is installed to WordPress, the list of the custom fields is automatically updated.', 'memsource'); ?></p>
        </div>
        <?php
    ...
    public function formSubmit()
    {
        ...
        foreach ($this->getItems() as $item) {
            $this->customFieldsSettingsService->saveContentSettings(
                $item['key'],
                $item['type'],
                isset($_POST[$item['hash']]) // phpcs:ignore
            );
        }
        $wpdb->query('COMMIT');
        wp_redirect($_POST['referer']); // phpcs:ignore
    }
